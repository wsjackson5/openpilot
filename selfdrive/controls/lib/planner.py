...
# make sure these accelerations are smaller than mpc limits
_A_CRUISE_MAX_V = [1.0, 1.0, 0.5, .3]
_A_CRUISE_MAX_V_FOLLOWING = [1.0, 1.0, 0.5, .3]
...
_AP_CRUISE_MIN_V_ECO = [-1.0, -0.7, -0.6, -0.5, -0.3]
_AP_CRUISE_MIN_V_SPORT = [-3.0, -2.6, -2.3, -2.0, -1.0]
_AP_CRUISE_MIN_V_FOLLOWING = [-5.0, -4.5, -4.0, -3.0, -2.0]
_AP_CRUISE_MIN_BP = [0.0, 5.0, 10.0, 20.0, 55.0]

_AP_CRUISE_MAX_V = [1.0, 1.0, 0.9, 0.5, 0.3]
_AP_CRUISE_MAX_V_ECO = [0.8, 0.8, 0.7, 0.4, 0.3]
_AP_CRUISE_MAX_V_SPORT = [1.2, 1.2, 1.1, .7, .5]
_AP_CRUISE_MAX_V_FOLLOWING = [1.0, 1.0, 0.9, 0.5, 0.3]
...
class Planner():
  ...
  def update(self, sm, pm, CP, VM, PP):
    ...
    if self.dragon_slow_on_curve and len(sm['model'].path.poly):
      ...
      a_y_max = 1.2 - v_ego * 0.03  # ~0.6 @ 72kph, ~0.9 @ 36kph
    ...
    if enabled and not self.first_loop and not sm['carState'].regenPressed:
